...
ifeq ($(ML_LOCAL_VER),$(ML_REMOTE_VER))
	...
	sudo rm -rf /usr/local/bin/mailer
	sudo cp -nf "${ML_ROOT_DIR}/mailer" /usr/local/bin/mailer
	...
	sudo rm -rf /etc/mailer/mail.html
	sudo cp -nf "${ML_ROOT_DIR}/mail.html" /etc/mailer/mail.html
